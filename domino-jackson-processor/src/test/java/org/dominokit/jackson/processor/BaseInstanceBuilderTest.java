...
public abstract class BaseInstanceBuilderTest extends TestCase {
  ...
  void runTests() throws Exception {
    ...
    testProcessor.setDelegate(
        (annotations, roundEnv, typeUtils, elementUtils, filer, messager) -> {
          Optional<? extends Element> element =
              roundEnv.getElementsAnnotatedWith(TestAnnotation.class).stream().findFirst();
          if (element.isPresent()) {
            ...
            Map<String, ? extends Element> elements =
                typeElement.getEnclosedElements().stream()
                    .filter(e -> ElementKind.FIELD.equals(e.getKind()))
                    .collect(Collectors.toMap(o -> o.getSimpleName().toString(), o -> o));
            testSubjects.forEach(
                testSubject -> {
                  if (elements.containsKey(testSubject.fieldName)) {
                    CodeBlock instance =
                        getMappersChainBuilder(element.get().asType())
                            .getInstance(elements.get(testSubject.fieldName));
    ...
    Compilation compilation =
        javac().withProcessors(testProcessor).compile(JavaFileObjects.forResource("TestBean.java"));
