...
import static org.dominokit.jackson.processor.AbstractMapperProcessor.elementUtils;
...
public class AptDeserializerBuilder extends AbstractJsonMapperGenerator {
  ...
  protected TypeName superClass() {
    return ParameterizedTypeName.get(
        ClassName.get(AbstractBeanJsonDeserializer.class), ClassName.get(beanType));
  }
  ...
  protected MethodSpec initSubtypesMethod() {
    if (subTypesInfo == null)
      ...
      MethodSpec.Builder builder =
          MethodSpec.methodBuilder("initMapSubtypeClassToDeserializer")
              .addModifiers(Modifier.PROTECTED)
              .addAnnotation(Override.class)
              .returns(
                  ParameterizedTypeName.get(
      ...
      for (Map.Entry<String, TypeMirror> subtypeEntry : subTypesInfo.getSubTypes().entrySet()) {
        ...
        String pkg =
            MoreElements.getPackage(MoreTypes.asTypeElement(subtypeEntry.getValue()))
                .getQualifiedName()
                .toString();
        TypeSpec subtypeType =
        ...
        builder.addStatement(
            "map.put($T.class, $L)", TypeName.get(subtypeEntry.getValue()), subtypeType);
      }
  ...
  protected Set<MethodSpec> moreMethods() {
    ...
    JsonIgnoreProperties ignorePropertiesAnnotation =
        typeUtils.asElement(beanType).getAnnotation(JsonIgnoreProperties.class);
  ...
  private MethodSpec buildInitIdentityInfoMethod(BeanIdentityInfo identityInfo) {
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder("initIdentityInfo")
            .addModifiers(Modifier.PROTECTED)
            .addAnnotation(Override.class)
            .returns(
                ParameterizedTypeName.get(
                    ClassName.get(IdentityDeserializationInfo.class), TypeName.get(beanType)));

    if (identityInfo.isIdABeanProperty()) {
      builder.addStatement(
          "return $L", buildPropertyIdentifierDeserializationInfo(beanType, identityInfo));
    } else {
      builder.addStatement(
          "return $L",
          buildIdentifierDeserializationInfo(
              beanType,
              identityInfo,
              new FieldDeserializersChainBuilder(identityInfo.getType().get())
                  .getInstance(identityInfo.getType().get())));
    }

    return builder.build();
  }

  private TypeSpec buildIdentifierDeserializationInfo(
      TypeMirror type, BeanIdentityInfo identityInfo, CodeBlock deserializerType) {
    return TypeSpec.anonymousClassBuilder(
            "$S, $T.class, $T.class",
            identityInfo.getPropertyName(),
            identityInfo.getGenerator(),
            identityInfo.getScope().get())
        .superclass(
            ParameterizedTypeName.get(
                ClassName.get(AbstractIdentityDeserializationInfo.class),
                TypeName.get(type),
                TypeName.get(identityInfo.getType().get())))
        .addMethod(
            MethodSpec.methodBuilder("newDeserializer")
                .addModifiers(Modifier.PROTECTED)
                .addAnnotation(Override.class)
                .returns(
                    ParameterizedTypeName.get(
                        ClassName.get(JsonDeserializer.class), DEFAULT_WILDCARD))
                .addStatement("return $L", deserializerType)
                .build())
        .build();
  }

  private CodeBlock buildPropertyIdentifierDeserializationInfo(
      TypeMirror type, BeanIdentityInfo identityInfo) {
    return CodeBlock.builder()
        .add(
            "new $T($S, $T.class, $T.class)",
            ParameterizedTypeName.get(
                ClassName.get(PropertyIdentityDeserializationInfo.class), TypeName.get(type)),
            identityInfo.getPropertyName(),
            identityInfo.getGenerator(),
            identityInfo.getScope().get())
        .build();
  ...
  private String getBuilderName() {
    JsonDeserialize jsonDeserialize =
        typeUtils.asElement(beanType).getAnnotation(JsonDeserialize.class);
  ...
  private MethodSpec buildIgnoreUnknownMethod(boolean ignored) {
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder("isDefaultIgnoreUnknown")
            .addAnnotation(Override.class)
            .addModifiers(Modifier.PROTECTED)
  ...
  private List<Element> getIgnoredFields(TypeElement typeElement) {
    ...
    final List<Element> ignoredFields =
        typeElement.getEnclosedElements().stream()
            .filter(e -> ElementKind.FIELD.equals(e.getKind()) && isNotStatic(e) && isIgnored(e))
  ...
  private MethodSpec buildInitInstanceBuilderMethod() {
    ParameterizedTypeName deserializersTypeName =
        ParameterizedTypeName.get(
            ClassName.get(MapLike.class), ClassName.get(HasDeserializerAndParameters.class));
    MethodSpec.Builder initInstanceMethodBuilder =
        MethodSpec.methodBuilder("initInstanceBuilder")
            .addModifiers(Modifier.PROTECTED)
            .returns(
                ParameterizedTypeName.get(
    ...
    if (isUseJsonCreator()) {
      ...
      parameterBuilders =
          parameterTypes.stream()
              ...
              .collect(Collectors.toList());
      initInstanceMethodBuilder.addStatement(
          "final $T deserializers = $T.get().mapLikeFactory().make()",
          deserializersTypeName,
          JacksonContextProvider.class);
      buildPropertiesDeserializers(initInstanceMethodBuilder);
    } else {
      initInstanceMethodBuilder.addStatement(
          "final $T deserializers = null", deserializersTypeName);
    }
    ...
    if (useBuilder) {
      ...
      initInstanceMethodBuilder.addStatement(
          "final $T builderDeserializer = new $T()",
          ParameterizedTypeName.get(
              ClassName.get(AbstractBeanJsonDeserializer.class), ClassName.get(builderElement)),
          builderDeserializerName(builderElement));
      try {
        new AptDeserializerBuilder(
                packageName, builderElement.asType(), AbstractMapperProcessor.filer)
            .generate();
      } catch (IOException e) {
  ...
  private void buildPropertiesDeserializers(MethodSpec.Builder initInstanceMethodBuilder) {
    for (ParameterDeserializerBuilder parameterBuilder : parameterBuilders) {
      ...
      initInstanceMethodBuilder.addStatement(
          "deserializers.put($S, $L)",
          parameterBuilder.getParameterName(),
          parameterBuilder.getDeserializerName());
    }
  }
  ...
  private TypeSpec instanceBuilderReturnType(boolean useBuilder) {
    ...
    if (isUseJsonCreator()) {
      ...
      createMethodBuilder.addStatement(
          newBeanBuilder.toString(), TypeName.get(typeUtils.erasure(beanType)));
    } else {
      createMethodBuilder.addStatement(
          (Type.isGenericType(beanType) ? "return new $T<>()" : "return new $T()"),
  ...
  private MethodSpec newInstanceMethod(
      TypeMirror beanType, MethodSpec createMethod, boolean useBuilder) {
    MethodSpec.Builder builder =
    ...
    if (useBuilder) {
      ...
      builder.addStatement(
          "return new $T(builderDeserializer.deserializeInline(reader, ctx, params, null, null, null, bufferedProperties)."
              + buildMethodName
              + "(), bufferedProperties)",
          ParameterizedTypeName.get(ClassName.get(Instance.class), ClassName.get(beanType)));
  ...
  private void buildAssignProperties(
      TypeMirror beanType, MethodSpec createMethod, MethodSpec.Builder builder) {
    for (ParameterDeserializerBuilder parameterBuilder : parameterBuilders) {
      builder.addStatement(
          "$T $L = null",
          Type.wrapperType(parameterBuilder.getParameterType()),
          parameterBuilder.getParameterName() + "Property");
    }
    ...
    for (ParameterDeserializerBuilder parameterBuilder : parameterBuilders) {
      ...
      builder.addStatement(
          "$L = $L.deserialize(reader, ctx)", paramName + "Property", paramName + "Deserializer");
  ...
  private Optional<MethodSpec> buildInitDeserializersMethod(TypeMirror beanType) {
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder("initDeserializers")
            .addModifiers(Modifier.PROTECTED)
            .addAnnotation(Override.class)
            .returns(resultType)
            .addStatement(
                "$T map = $T.get().mapLikeFactory().make()",
                resultType,
                JacksonContextProvider.class);
        ...
        .forEach(
            entry ->
                builder.addStatement(
                    ...
                    new DeserializerBuilder(typeUtils, beanType, entry.getKey(), entry.getValue())
                        .buildDeserializer()));
  ...
  /**
   * @param field
   * @return the field provided in the {@link JsonProperty} as long as the provided name is not
   *     JsonProperty.USE_DEFAULT_NAME otherwise return the field simple name
   */
