...
import org.dominokit.jackson.ser.bean.*;
import org.dominokit.jackson.ser.bean.SubtypeSerializer.BeanSubtypeSerializer;
...
public class AptSerializerBuilder extends AbstractJsonMapperGenerator {
  ...
  protected TypeName superClass() {
    return ParameterizedTypeName.get(
        ClassName.get(AbstractBeanJsonSerializer.class), ClassName.get(beanType));
  }
  ...
  private Optional<CodeBlock> getIdentitySerializerType(BeanIdentityInfo identityInfo) {
    if (identityInfo.isIdABeanProperty()) {
      ...
      return Optional.of(
          new FieldSerializerChainBuilder(identityInfo.getType().get())
              .getInstance(identityInfo.getType().get()));
    }
  }

  private MethodSpec buildInitIdentityInfoMethod(
      Optional<CodeBlock> serializerType, Optional<BeanIdentityInfo> beanIdentityInfo) {
    return MethodSpec.methodBuilder("initIdentityInfo")
        .addModifiers(Modifier.PROTECTED)
        .addAnnotation(Override.class)
        .returns(
            ParameterizedTypeName.get(
                ClassName.get(IdentitySerializationInfo.class), TypeName.get(beanType)))
        .addStatement(
            "return $L",
  ...
  private TypeSpec generateIdentifierSerializationInfo(
      BeanIdentityInfo identityInfo, Optional<CodeBlock> serializerType) {

    TypeSpec.Builder builder =
        TypeSpec.anonymousClassBuilder(
            "$L, $S", identityInfo.isAlwaysAsId(), identityInfo.getPropertyName());

    if (identityInfo.isIdABeanProperty()) {

      Map<Element, TypeMirror> fieldsMap = orderedFields();
      Optional<Map.Entry<Element, TypeMirror>> propertyEntry =
              ...
              .filter(
                  entry ->
                      entry
                          ...
                          .getSimpleName()
                          .toString()
                          .equals(identityInfo.getPropertyName()))
              .findFirst();
      if (propertyEntry.isPresent()) {
        builder.superclass(
            ParameterizedTypeName.get(
                ClassName.get(PropertyIdentitySerializationInfo.class),
                TypeName.get(beanType),
                Type.wrapperType(propertyEntry.get().getValue())));
      ...
      builder.superclass(
          ParameterizedTypeName.get(
              ClassName.get(AbstractIdentitySerializationInfo.class),
              TypeName.get(beanType),
              TypeName.get(qualifiedType)));
      ...
      builder.addMethod(
          MethodSpec.methodBuilder("newSerializer")
              ...
              .addAnnotation(Override.class)
              .returns(
                  ParameterizedTypeName.get(ClassName.get(JsonSerializer.class), DEFAULT_WILDCARD))
              .addStatement("return $L", serializerType.get())
              .build());

      TypeName generatorType =
          ParameterizedTypeName.get(
              ClassName.get(ObjectIdGenerator.class), TypeName.get(qualifiedType));
      TypeName returnType =
          ParameterizedTypeName.get(
              ClassName.get(ObjectIdSerializer.class), TypeName.get(qualifiedType));

      builder.addMethod(
          MethodSpec.methodBuilder("getObjectId")
              ...
              .addStatement(
                  "$T generator = new $T().forScope($T.class)",
                  generatorType,
              ...
              .endControlFlow()
              .addStatement(
                  "return new $T(scopedGen.generateId(bean), getSerializer())", returnType)
              .build());
    }
  ...
  private void buildBeanPropertySerializerBody(
      TypeSpec.Builder builder, Map.Entry<Element, TypeMirror> property) {
    String paramName = "bean";
    AccessorInfo accessorInfo =
        new SerializerBuilder(typeUtils, beanType, property.getKey(), property.getValue())
            .getterInfo();

    MethodSpec.Builder newSerializerMethodBuilder =
        MethodSpec.methodBuilder("newSerializer")
            .addModifiers(Modifier.PROTECTED)
            .addAnnotation(Override.class)
            .addStatement(
                "return $L",
                new FieldSerializerChainBuilder(property.getValue())
                    .getInstance(property.getValue()));
    newSerializerMethodBuilder.returns(
        ParameterizedTypeName.get(ClassName.get(JsonSerializer.class), DEFAULT_WILDCARD));
  ...
  protected MethodSpec initSubtypesMethod() {
    if (subTypesInfo == null) {
      ...
      MethodSpec.Builder builder =
          MethodSpec.methodBuilder("initMapSubtypeClassToSerializer")
              .addModifiers(Modifier.PROTECTED)
              .addAnnotation(Override.class)
              .returns(
                  ParameterizedTypeName.get(
            ...
            TypeSpec.anonymousClassBuilder("")
                ...
                .addMethod(
                    MethodSpec.methodBuilder("newSerializer")
                        ...
                        .addStatement(
                            "return new $L()",
                            ClassName.bestGuess(Type.serializerName(pkg, subtypeEntry.getValue())))
                        .build())
                .build();
  ...
  private Optional<MethodSpec> buildInitSerializersMethod(TypeMirror beanType) {
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder("initSerializers")
            .addModifiers(Modifier.PROTECTED)
            .addAnnotation(Override.class)
            .returns(TypeName.get(BeanPropertySerializer[].class))
            .addStatement(
                "$T result = new $T[$L]",
                ArrayTypeName.of(BeanPropertySerializer.class),
                BeanPropertySerializer.class,
                fields.size());
        ...
        .forEach(
            entry ->
                builder.addStatement(
                    "result[$L] = $L",
                    index[0]++,
                    new SerializerBuilder(typeUtils, beanType, entry.getKey(), entry.getValue())
                        .buildSerializer()));
