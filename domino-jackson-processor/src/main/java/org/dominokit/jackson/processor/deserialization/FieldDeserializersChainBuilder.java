...
public class FieldDeserializersChainBuilder implements MappersChainBuilder {
  ...
  private String getFieldDeserializer(TypeMirror typeMirror) {
    ...
    if (Type.isIterable(typeMirror)) return getIterableDeserializer(typeMirror);
    if (Type.isMap(typeMirror)) return getMapDeserializer(typeMirror);
    if (Type.isArray(typeMirror)) return getArrayDeserializer(typeMirror);
    if (Type.isEnum(typeMirror)) return getEnumDeserializer(typeMirror);
    return getBasicOrCustomDeserializer(typeMirror);
  }

  private String getBasicOrCustomDeserializer(TypeMirror typeMirror) {
    if (Type.isBasicType(typeMirror)) return getBasicDeserializer(typeMirror);
  ...
  private String getCustomDeserializer(TypeMirror typeMirror) {
    if (Type.stringifyTypeWithPackage(typeMirror).equals(Type.stringifyTypeWithPackage(beanType))
        && rootGenerated) {
      deserializers.addLast(
          ClassName.bestGuess(Type.deserializerName(getPackageName(typeMirror), typeMirror)));
    } else {
      if (TypeRegistry.containsDeserializer(Type.stringifyTypeWithPackage(typeMirror))) {
        ...
        TypeRegistry.registerDeserializer(
            ...
            ClassName.bestGuess(generateCustomDeserializer(typeMirror)));
        deserializers.addLast(TypeRegistry.getCustomDeserializer(typeMirror));
        rootGenerated = true;
      }
    }
  ...
  private String generateCustomDeserializer(TypeMirror typeMirror) {
    if (TypeRegistry.containsDeserializer(typeMirror)
        || TypeRegistry.isInActiveGenDeserializer(typeMirror)) {
  ...
  private String getMapDeserializer(TypeMirror typeMirror) {
    ...
    return NEW_INSTANCE
        + getKeyDeserializer(Type.firstTypeArgument(typeMirror))
        + ", "
        + getFieldDeserializer(Type.secondTypeArgument(typeMirror))
        + ")";
  }

  private String getKeyDeserializer(TypeMirror typeMirror) {
    typeMirror = Type.removeOuterWildCards(typeMirror);
    if (Type.isEnum(typeMirror)) return getEnumKeyDeserializer(typeMirror);
    return getBasicKeyDeserializer(typeMirror);
  }

  private String getBasicKeyDeserializer(TypeMirror typeMirror) {
    deserializers.addLast(
        TypeRegistry.getKeyDeserializer(
  ...
  private String getArrayDeserializer(TypeMirror typeMirror) {
    if (Type.isPrimitiveArray(typeMirror)) {
      ...
      return NEW_INSTANCE
          + getFieldDeserializer(Type.deepArrayComponentType(typeMirror))
          + ", "
          + getArray2dCreatorFormat(typeMirror)
          + ")";
    } else {
      deserializers.addLast(TypeName.get(ArrayJsonDeserializer.class));
      return NEW_INSTANCE
          + getFieldDeserializer(Type.arrayComponentType(typeMirror))
          + ", "
          + getArrayCreatorFormat(typeMirror)
          + ")";
    }
  }
  ...
  private String getArrayCreatorFormat(TypeMirror typeMirror) {
    ...
    deserializers.addLast(
        TypeName.get(ObjectMapperProcessor.typeUtils.erasure(Type.arrayComponentType(typeMirror))));
    return "($T<$T>) $T[]::new";
  }
  ...
  private String getArray2dCreatorFormat(TypeMirror typeMirror) {
    ...
    deserializers.addLast(
        TypeName.get(
            ObjectMapperProcessor.typeUtils.erasure(Type.deepArrayComponentType(typeMirror))));
