...
/**
 * Annotation that can be used to either suppress serialization of properties (during
 ...
 * </pre>
 *
 * <p>Annotation can be applied both to classes and to properties. If used for both, actual set will
 * be union of all ignorals: that is, you can only add properties to ignore, not remove or override.
 * So you can not remove properties to ignore using per-property annotation.
 */
@Target({
...
public @interface JsonIgnoreProperties {
  ...
  /**
   * Property that can be enabled to allow "getters" to be used (that is, prevent ignoral of getters
   * for properties listed in {@link #value()}). This is commonly set to support defining
   * "read-only" properties; ones for which there is a getter, but no matching setter: in this case,
   * properties should be ignored for deserialization but NOT serialization. Another way to think
   * about this setting is that setting it to `true` will "disable" ignoring of getters.
   *
  ...
  /**
   * Property that can be enabled to allow "setters" to be used (that is, prevent ignoral of setters
   * for properties listed in {@link #value()}). This could be used to specify "write-only"
   * properties; ones that should not be serialized out, but that may be provided in for
   * deserialization. Another way to think about this setting is that setting it to `true` will
   * "disable" ignoring of setters.
   *
  ...
  public static class Value
      implements JacksonAnnotationValue<JsonIgnoreProperties>, java.io.Serializable {
    private static final long serialVersionUID = 1L;

    /**
     * Default instance has no explicitly ignored fields, does not ignore unknowns, does not
     * explicitly allow getters/setters (that is, ignorals apply to both), but does use merging for
     * combining overrides with base settings
     */
    protected static final Value EMPTY =
        new Value(Collections.<String>emptySet(), false, false, false, true);
    ...
    /**
     * Factory method that may be used (although is NOT the recommended way) to construct an
     ...
     *   <li>Does not "allow" getters if property ignored (that is, ignorals apply to both setter
     *       and getter)
     *   <li>Does not "allow" setters if property ignored (that is, ignorals apply to both setter
     *       and getter)
