...
/**
 * Base implementation of {@link org.dominokit.jackson.deser.map.key.KeyDeserializer} for dates. It
 * uses both ISO-8601 and RFC-2822 for string-based key and milliseconds for number-based key.
 */
public abstract class BaseDateKeyDeserializer<D extends Date> extends KeyDeserializer<D>
  ...
  protected D doDeserialize(String key, JsonDeserializationContext ctx) {
    return ((DateKeyParser<D>) JacksonContextProvider.get().dateFormat().makeDateKeyParser())
        .parse(key, this);
  }
}
