 ...
 *
 * <p>Note that the main inclusion criteria (one annotated with {@link #value}) is checked on
 * <b>Java object level</b>, for the annotated type, and <b>NOT</b> on JSON output -- so even with
 ...
 * instance constructed to reference <code>null</code> value: such a value would be serialized as
 * JSON null, and not filtered out.
 *
 * <p>To base inclusion on value of contained value(s), you will typically also need to specify
 * {@link #content()} annotation; for example, specifying only {@link #value} as {@link
 * Include#NON_EMPTY} for a {link java.util.Map} would exclude <code>Map</code>s with no values, but
 * would include <code>Map</code>s with `null` values. To exclude Map with only `null` value, you
 ...
 *
 * <p>In addition to `Map`s, `content` concept is also supported for referential types (like {@link
...
public @interface JsonInclude {
   ...
   * so, an instance is created by calling <code>HandlerInstantiator</code> (of <code>ObjectMapper
  ...
  public enum Include {
     ...
     * actual handling.
     *
     * <p>Default emptiness for all types includes:
     *
     * <ul>
     *   <li><code>Null</code> values.
     *   <li>"Absent" values (see {@link #NON_ABSENT})
     * </ul>
     *
     * so that as baseline, "empty" set includes values that would be excluded by both {@link
     * #NON_NULL} and {@link #NON_ABSENT}. <br>
     * Beyond this base, following types have additional empty values:
     *
     * <ul>
     *   <li>For {@link java.util.Collection}s and {@link java.util.Map}s, method <code>isEmpty()
     *       </code> is called;
     *   <li>For Java arrays, empty arrays are ones with length of 0
     *   <li>For Java {@link String}s, <code>length()</code> is called, and return value of 0
     *       indicates empty String
     * </ul>
     *
     * and for other types, null values are excluded but other exclusions (if any).
     *
     * <p>Note that this default handling can be overridden by custom <code>JsonSerializer</code>
     * implementation: if method <code>isEmpty()</code> is overridden, it will be called to see if
     * non-null values are considered empty (null is always considered empty).
     *
     * <p>Compatibility note: Jackson 2.6 included a wider range of "empty" values than either
     ...
     * property override.
     *
     * <p>When used for a POJO, definition is that only values that differ from the default values
     ...
     * by using <code>equals()</code> method, except for the case where property has `null` value in
     * which case straight null check is used.
     *
     * <p>When NOT used for a POJO (that is, as a global default, or as property override),
     * definition is such that:
     *
     * <ul>
     *   <li>All values considered "empty" (as per {@link #NON_EMPTY}) are excluded
     *   <li>Primitive/wrapper default values are excluded
     *   <li>Date/time values that have timestamp (`long` value of milliseconds since epoch, see
     ...
     * object's <code>equals()</code> method is called with value to serialize; if it returns <code>
     * true</code> value is <b>excluded</b> (that is, filtered out); if <code>false</code> value is
     * <b>included</b>.
     *
     * @since 2.9
     */
  ...
  public static class Value
     ...
     *
     * <ul>
     *   <li>Set <code>value</code> as <code>USE_DEFAULTS</code> and <code>valueFilter</code> to
     *       <code>filter</code> (if filter not null); or
     ...
     *
     * <ul>
     *   <li>Set <code>content</code> as <code>USE_DEFAULTS</code> and <code>contentFilter</code> to
     *       <code>filter</code> (if filter not null); or
