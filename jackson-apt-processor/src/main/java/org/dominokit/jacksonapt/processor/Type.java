...
public class Type {
  ...
  public static boolean isEnum(TypeMirror typeMirror) {
    return !isNull(ObjectMapperProcessor.typeUtils.asElement(typeMirror))
        ...
        && ElementKind.ENUM.compareTo(
                ObjectMapperProcessor.typeUtils.asElement(typeMirror).getKind())
            == 0;
  }
  ...
  public static String getPackage(TypeMirror typeMirror) {
    return typeUtils.asElement(typeUtils.erasure(typeMirror)) != null
        ? elementUtils
            .getPackageOf(typeUtils.asElement(typeUtils.erasure(typeMirror)))
            .getQualifiedName()
            .toString()
        : "";
  }
            ...
            return t.getExtendsBound() != null
                ...
                : t.getSuperBound() != null
                    ...
                    : typeUtils.getDeclaredType(
                        elementUtils.getTypeElement(Object.class.getName()));
          }
   ...
   */
  public static TypeMirror getDeclaredType(
      TypeMirror type,
  ...
  /**
   * Check if given type is generic class (and not being collection, iterable, enum or map) with
   * type argument containing bounded wildcard.
   *
   * <p>All type parameters of the type needs to be resolved to actual type arguments prior calling
   * this method. Note that presence of type parameter causes RuntimeException.
   *
   * @param type {@link javax.lang.model.type.TypeMirror} to be checked.
  ...
  public static boolean hasTypeArgumentWithBoundedWildcards(TypeMirror type) {
    return type.accept(
        new SimpleTypeVisitor8<Boolean, Boolean>() {
          @Override
          public Boolean visitDeclared(DeclaredType t, Boolean p) {
            final Boolean isCustomType =
                !Type.isCollection(t) && !Type.isIterable(t) && !Type.isMap(t) && !Type.isEnum(t);
          ...
          public Boolean visitWildcard(WildcardType t, Boolean p) {
            return t.getExtendsBound() != null
                ...
                : t.getSuperBound() != null ? p || !visit(t.getSuperBound(), p) : false;
          }
  ...
  public static Optional<TypeMirror> findClassValueFromClassAnnotation(
    ...
    if (superclass.getKind().equals(TypeKind.NONE)) {
      ...
      return findClassValueFromClassAnnotation(
          typeUtils.asElement(superclass), annotation, paramName);
    }
  }
  ...
  public static Optional<BeanIdentityInfo> processIdentity(TypeMirror beanType) {
    ...
    Optional<TypeMirror> generatorClass =
        findClassValueFromClassAnnotation(element, JsonIdentityInfo.class, "generator");

    if (nonNull(jsonIdentityInfoAnno)
        && !isAssignableFrom(generatorClass.get(), ObjectIdGenerators.None.class)) {

      JsonIdentityReference jsonIdentityReferenceAnno =
          findClassAnnotation(element, JsonIdentityReference.class);

      String propertyName = jsonIdentityInfoAnno.property();
      boolean alwaysAsId =
          nonNull(jsonIdentityReferenceAnno) && jsonIdentityReferenceAnno.alwaysAsId();
      TypeMirror generator =
          findClassValueFromClassAnnotation(element, JsonIdentityInfo.class, "generator").get();
      Optional<TypeMirror> scope =
          findClassValueFromClassAnnotation(element, JsonIdentityInfo.class, "scope");
      if (!scope.isPresent()) {
      ...
      if (isAssignableFrom(
          typeUtils.asElement(generator), ObjectIdGenerators.PropertyGenerator.class)) {
        ...
        if (typeUtils.isSameType(
            generator, toType(ObjectIdGenerators.IntSequenceGenerator.class))) {
          identityType = Optional.of(toType(Integer.class));
        } else if (typeUtils.isSameType(
            generator, toType(ObjectIdGenerators.UUIDGenerator.class))) {
          ...
          TypeMirror objectIdGeneratorType =
              typeUtils.erasure(
                  elementUtils.getTypeElement(ObjectIdGenerator.class.getCanonicalName()).asType());
        ...
        beanIdentityInfo =
            new BeanIdentityInfo(propertyName, alwaysAsId, generator, scope, identityType);
      }
